...
def create_dr():
    ...
    hashedIdentifier_analkanal = create_observation(ident=patID9+"-"+str(get_xml_values.get_id(root)), result=get_values_rektum.get_analkanal(root),
    ...
    hashedIdentifier_MRF = create_observation(ident=patID9+"-"+str(get_xml_values.get_id(root)), result=get_values_rektum.get_MRF(root), code=("408655002",
    ...
    hashedIdentifier_EMVI = create_observation(ident=patID9+"-"+str(get_xml_values.get_id(root)), result=get_values_rektum.get_EMVI(root), code=("1286755009",
    ...
    hashedIdentifier_oberrand = create_observation(ident=patID9+"-"+str(get_xml_values.get_id(root)), result=get_values_rektum.get_höhe_tumor_oberrand(root),
    ...
    hashedIdentifier_unterrand = create_observation(ident=patID9+"-"+str(get_xml_values.get_id(root)), result=get_values_rektum.get_höhe_tumor_unterrand(root),
    ...
    hashedIdentifier_TNM = create_observation(ident=patID9+"-"+str(get_xml_values.get_id(root)), result=get_values_rektum.get_TNM(root), code=("260879005",
...
def create_observation(ident, result, code, patientRef, acsn, type, text=None, sop_instance_available=False):
    ...
    if sop_instance_available == True:
        try:
            sop_instance = get_values_rektum.get_höhe_tumor_unterrand(root)[2]
        except Exception:
            pass
